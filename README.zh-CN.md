...
![Docker-VPN 先开放本地 HKU 代理，再由应用直接使用或通过分流规则使用](docs/images/split-routing-zh.png)
...
`121.37.195.197/32` 不是示例地址，而是 `hkuvpn cn` 当前使用的大陆 VPN 入口。如果 HKU 更换入口，应同时更新 [`bin/hkuvpn`](bin/hkuvpn) 和这条规则。

相比之下，`10.0.0.0/8` 在这里仅作为“不要整段分流”的 RFC 1918 反例，不是供复制的校园网段。家庭、公司和容器网络经常也使用该地址段；只添加服务实际使用的最小校园网段。
...
![学校电脑上的 Coding Agent 通过 Reverse SSH 使用客户端代理](docs/images/reverse-ssh-zh.png)
